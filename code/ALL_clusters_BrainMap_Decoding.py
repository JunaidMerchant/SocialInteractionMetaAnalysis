...
out_dir1 = os.path.abspath("./results/decode/bmap_ALLclusters/")
...
data_sets = 'BrainMap_dataset'
d = 'BrainMap_dataset'


# load dataset
NS = Dataset.load(ds_dir + data_sets + ".pkl.gz")
...
for r in range(len(roi_files)):
    ...
    # if os.path.isfile(out_dir + "/" + roiname + "_ROIAssociationDecoder_" + d + ".csv"):
    #     continue
    ...
    if not os.path.isfile(out_dir + "/" + roiname + "_BrainMapDecoder_" + d + ".csv"):
        ...
        decoded_df.to_csv(out_dir + "/" + roiname + "_BrainMapDecoder_" + d + ".csv")


    # Run NeurosynthDecoder discrete
    if not os.path.isfile(out_dir + "/" + roiname + "_NeurosynthDecoder_" + d + ".csv"):
        print('working on NeurosynthDecoder')
        decoder = discrete.NeurosynthDecoder()
        decoder.fit(NS)
        decoded_df = decoder.transform(ids=AllRoiStudies)
        decoded_df.sort_values(by="zReverse", ascending=False,inplace=True)
        decoded_df.to_csv(out_dir + "/" + roiname + "_NeurosynthDecoder_" + d + ".csv")

    # Run ROIAssociationDecoder discrete
    if not os.path.isfile(out_dir + "/" + roiname + "_ROIAssociationDecoder_" + d + ".csv"):
        ...
        decoded_df.to_csv(out_dir + "/" + roiname + "_ROIAssociationDecoder_" + d + ".csv")
